...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    res = [paragraph for paragraph in paragraphs if select(paragraph)]
    if k >= len(res):
        return ''
    return res[k]
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    i, j = len(typed_words), len(reference_words)
    if i == 0:
        return 0.0
    else:
        correct = 0
...
def fastest_words(game):
    ...
    for i in words:
        time_list = []
        for t in players:
            time_list.append(time(game, t, i))
        min_index = time_list.index(min(time_list))
        res[min_index].append(word_at(game, i))
