...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operand = rest.map(lambda val: scheme_eval(val, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        value = do_lambda_form(expression, env)
...
def make_let_frame(bindings, env):
    ...
    def names_helper(expressions):
        if expressions == nil:
            return nil
        binding = expressions.first
        validate_form(binding, 2, 2)
        return Pair(binding.first, names_helper(expressions.rest))

    def values_helper(expressions):
        if expressions == nil:
            return nil
        binding = expressions.first
        return Pair(eval_all(binding.rest, env), values_helper(expressions.rest))
    
    names = names_helper(bindings)
    values = values_helper(bindings)
